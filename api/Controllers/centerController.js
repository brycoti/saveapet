...
const newPet = async (req, res, next, Center, Pet) => {
  try {
    const center = await Center.findByPk(); // Cerca l'usuari pel seu ID
  ...
  const login2 = async (req, res, Model) => {
    try {
        ...
        res.json({ name: user.name, userId: user.id, email: user.email, phonenumber : user.phonenumber, web: user.web, city: user.city, address: user.address}); // Retorna missatge d'èxit
      } catch (error) {
